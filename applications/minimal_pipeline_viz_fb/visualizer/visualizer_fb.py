#!/usr/bin/env python3
"""
Minimal Visualizer with FlatBuffers Deserialization
"""
        ...
        async def sub():
            async def message_handler(msg):
                try:
                    ...
                    fb_msg = Message.Message.GetRootAs(msg.data, 0)
                    unique_id = fb_msg.UniqueId().decode()
                    
                    # Convert to numpy
                    array = fb_to_numpy(fb_msg.Payload())
                    
                    # Store
                    self.messages[unique_id] = {
                        'data': array,
...
app.layout = html.Div([
    html.H1("Ising Visualizer (FlatBuffers)", style={'textAlign': 'center'}),
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("Ising Visualizer (FlatBuffers)")
    print("="*60)
